...
class TestGameState(TestCase):
    def setUp(self) -> None:
        ...
        self.swarm_size = self.state.swarm_size

    def test_init_game(self) -> None:
        round_data_raw = torch.randint(0, 100, (self.batch_size, 3))
        round_data = [(round_data_raw[i][0],round_data_raw[i][1],round_data_raw[i][2]) for i in range(len(round_data_raw.tolist()))]
        ...
        for agent in self.state.trees:
            ...
            for batch in range(self.batch_size):
                self.assertEqual(self.state.trees[agent][batch].metadata['root_node']['environment_states'], round_data[batch][0])
                self.assertEqual(self.state.trees[agent][batch].metadata['root_node']['opponent_states'], round_data[batch][1])
                self.assertEqual(self.state.trees[agent][batch].metadata['root_node']['personal_states'], round_data[batch][2])
    ...
    def test_get_stage_state(self) -> None:
        ...
        for agent in range(self.swarm_size):
            ...
            for batch in range(self.batch_size):
                ...
                self.assertEqual(states[agent][batch][0][0], round_data[batch][0])
                self.assertEqual(states[agent][batch][0][1], round_data[batch][1])
                self.assertEqual(states[agent][batch][0][2], round_data[batch][2])
    ...
    def test_append_generation(self) -> None:
        ...
        agent_actions = {agent:[[actions for node in self.state.trees[agent][batch][0]] for batch in range(self.batch_size)] for agent in range(self.swarm_size)}
    ...
    def test_advance_stage(self) -> None:
        ...
        world_states = {agent:[[(new_stage_data[batch][0],new_stage_data[batch][1],new_stage_data[batch][2]) for node in self.state.trees[agent][batch][1]] for batch in range(self.batch_size)] for agent in range(self.swarm_size)} 
